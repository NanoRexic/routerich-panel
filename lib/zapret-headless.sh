...
PAUSE() { [ -z "$ZAPRET_HEADLESS" ] && { echo -ne "Нажмите Enter..."; read dummy; }; }
_headless_finish() { [ "${1:-0}" != "1" ] && PAUSE; return 0; }
BACKUP_DIR="/opt/zapret_backup"; DATE_FILE="$BACKUP_DIR/date_backup.txt"
...
get_ver() { URL="$1"; OUT_FILE="$2"; NAME="$3"; RESULT=$(curl -sIL --connect-timeout 2 --max-time 2 --retry 1 -w "%{url_effective}" -o /dev/null "$URL" 2>/dev/null); if [ $? -ne 0 ] || [ -z "$RESULT" ]; then return 1; fi; VERSION="${RESULT##*/}"; VERSION="${VERSION#v}"; [ -n "$VERSION" ] || return 1; echo "$VERSION" > "$OUT_FILE"; }
fetch_latest_versions() { TMP_VER="/tmp/zapret_version"; TMP_VER_POD="/tmp/podkop_version"; TMP_VER_GO="/tmp/tg_ws_proxy_go_ver"; get_ver "https://github.com/yandexru45/netshift/releases/latest" "$TMP_VER_POD" "NetShift" & get_ver "https://github.com/remittor/zapret-openwrt/releases/latest" "$TMP_VER" "Zapret" & get_ver "https://github.com/spatiumstas/tg-ws-proxy-go/releases/latest" "$TMP_VER_GO" "TG-WS Proxy GO" & wait; [ -s "$TMP_VER" ] && ZAPRET_VERSION="$(cat "$TMP_VER")"; [ -s "$TMP_VER_POD" ] && PODKOP_LATEST_VER="$(cat "$TMP_VER_POD")"; [ -s "$TMP_VER_GO" ] && GO_VER="$(cat "$TMP_VER_GO")"; }
...
switch_Dv_num() {
	NEW_NUM="$1"
	SELECTED_dv="Dv$NEW_NUM"
	eval NEW_STRAT=\$"$SELECTED_dv"
	grep -q -E '^[[:space:]]*--filter-tcp=2053,2083,2087,2096,8443' "$CONF" || return 1
	START=$(grep -n -E '^[[:space:]]*--filter-tcp=2053,2083,2087,2096,8443' "$CONF" | cut -d: -f1)
	END=$(tail -n +"$START" "$CONF" | grep -n -m1 -E '^--new$|^\x27$' | cut -d: -f1)
	END=$((START + END - 1))
	sed -i "${START},$((END-1))d" "$CONF"
	LINE=$START
	echo "$NEW_STRAT" | while IFS= read -r l; do sed -i "${LINE}i$l" "$CONF"; LINE=$((LINE + 1)); done
	if grep -q -E '^#[[:space:]]*Dv' "$CONF"; then sed -i "s/^#[[:space:]]*Dv[0-9]\+/#Dv$NEW_NUM/" "$CONF"; else sed -i "$START i#Dv$NEW_NUM" "$CONF"; fi
	ZAPRET_RESTART
	return 0
}
...
install_discord_script() {
	script="$1"
	case "$script" in
		50-stun4all) URL="https://raw.githubusercontent.com/bol-van/zapret/master/init.d/custom.d.examples.linux/50-stun4all" ;;
		50-quic4all) URL="https://raw.githubusercontent.com/bol-van/zapret/master/init.d/custom.d.examples.linux/50-quic4all" ;;
		50-discord-media) URL="https://raw.githubusercontent.com/bol-van/zapret/master/init.d/custom.d.examples.linux/50-discord-media" ;;
		50-discord) URL="https://raw.githubusercontent.com/bol-van/zapret/master/init.d/custom.d.examples.linux/50-discord" ;;
		*) return 1 ;;
	esac
	wget -q -U "Mozilla/5.0" -O "$CUSTOM_DIR/50-script.sh" "$URL" || return 1
	sed -i "/DISABLE_CUSTOM/s/'1'/'0'/" $CONF
	ZAPRET_RESTART
	return 0
}
...
_check_one_url_quiet() {
	...
	if curl -sL --connect-timeout 4 --max-time 6 --speed-time 3 --speed-limit 1 --range 0-65535 -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64) curl/8.0" -o /dev/null "$LINK" >/dev/null 2>&1; then
		echo "OK|$TEXT" >> "$LOG_TMP"
		echo 1 >> "$TMP_OK"
	else
		echo "FAIL|$TEXT" >> "$LOG_TMP"
	fi
}

test_urls_quiet() {
	TMP_OK="$TMP_SF/z_ok.$$"
	LOG_TMP="${LOG_TMP:-$TMP_SF/zapret_test_log.$$}"
	: > "$TMP_OK"
	: > "$LOG_TMP"
	OK=0
	RUN=0
...
build_domain_test_json() {
	STATUS=$(test_status_level)
	DOMAINS_JSON=$(jq -R -s '
		split("\n")
		| map(select(length > 0))
		| map(split("|"))
...
test_single_strategy_api() {
	...
	BLOCK=$(_extract_strategy_block "$SOURCE" "$NAME")
	[ -n "$BLOCK" ] || { rm -f "$BACK"; return 1; }

	_apply_strategy_block_to_conf "$BLOCK"
	ZAPRET_RESTART

	if ! prepare_urls; then
		_restore_conf_after_test
		rm -f "$BACK" "$OUT_DPI"
		return 1
	fi
	...
	rm -f "$OUT_DPI"
	LOG_TMP="$TMP_SF/zapret_api_strategy.$$"
	test_urls_quiet

	_restore_conf_after_test
	rm -f "$BACK"

	TEST_STRATEGY_NAME="$NAME"
...
test_current_strategy_api() {
	...
	TOTAL=$(grep -c "|" "$OUT_DPI")
	rm -f "$OUT_DPI"
	LOG_TMP="$TMP_SF/zapret_api_current.$$"
	test_urls_quiet || return 1
	build_domain_test_json
}
...
test_domains_api() {
	...
	[ "$COUNT" -gt 0 ] || return 1
	TOTAL="$COUNT"
	LOG_TMP="$TMP_SF/zapret_api_domain.$$"
	test_urls_quiet || return 1
	build_domain_test_json
}

clear_test_results_api() {
	rm -f /opt/zapret/tmp/results_flowseal.txt /opt/zapret/tmp/results_versions.txt /opt/zapret/tmp/results_all.txt /opt/zapret/tmp/results_domain.txt 2>/dev/null
	return 0
}
...
moonlight_insert_stub() {
	local line tmp="/tmp/moonlight_stub.$$"
	grep -q '^#Moonlight$' "$CONF" 2>/dev/null && return 0
	line=$(grep -n "^[[:space:]]*option NFQWS_OPT '" "$CONF" | head -1 | cut -d: -f1)
	[ -n "$line" ] || return 1
	strategy_moonlight_stub > "$tmp"
	sed -i "${line}r $tmp" "$CONF"
	rm -f "$tmp"
}

moonlight_remove_stub() {
	sed -i "/^#Moonlight\$/,/^--filter-tcp=${MOONLIGHT_TCP_PORTS}\$/d" "$CONF"
}

moonlight_cleanup_legacy_gv() {
...
echo -e "${GREEN}Стратегия для игр удалена!${NC}\n"; [ -z "$NO_PAUSE" ] && PAUSE; return 0; fi; if [ "$GAME_CHOICE" -eq 1 ]; then STRATEGY="$(strategy_Gv1; strategy_TCP_common)"; else STRATEGY="$(strategy_Gv "$GAME_CHOICE"; strategy_TCP_common)"; fi
...
if [ -z "$ANSWER" ]; then case "$CURRENT_NAME" in \#*) echo "$CURRENT_NAME" > "$SAVED_STR" ;; *) echo "#$CURRENT_NAME" > "$SAVED_STR" ;; esac; printf "%b\n" "$CURRENT_BODY" >> "$SAVED_STR"; echo -e "${CYAN}Применяем стратегию и перезапускаем ${NC}Zapret"
...
echo -en "Enter${GREEN} - применить стратегию, ${NC}S/s${GREEN} - остановить, ${NC}N/n${GREEN} - продолжить тест:${NC} "; read -r ANSWER </dev/tty; if [ -z "$ANSWER" ]; then case "$CURRENT_NAME" in \#*) echo "$CURRENT_NAME" > "$SAVED_STR" ;; *) echo "#$CURRENT_NAME" > "$SAVED_STR" ;; esac; printf "%b\n" "$CURRENT_BODY" >> "$SAVED_STR"; echo -e "${CYAN}Применяем стратегию и перезапускаем ${NC}Zapret"
...
strategy_CHOUSE () { echo -ne "\n${YELLOW}Введите версию стратегии (${NC}1-10${YELLOW}):${NC} "; read -r choice; case "$choice" in [1-9]|10) install_strategy "v$choice" ;; esac; }
_detect_base_ver() { local i c; [ -f "$CONF" ] || return; for i in $(seq 99 -1 1); do grep -qx "#v$i" "$CONF" && { echo "v$i"; return; }; done
c=$(grep -oE -- '--comment=Strategy__?v[0-9]+' "$CONF" 2>/dev/null | head -n1); [ -n "$c" ] && { echo "$c" | grep -oE 'v[0-9]+' | head -n1; return; }
c=$(grep -oE 'Strategy_v[0-9]+' "$CONF" 2>/dev/null | head -n1); [ -n "$c" ] && echo "$c" | grep -oE 'v[0-9]+' | head -n1; }
_detect_youtube_ver() { local i c; [ -f "$CONF" ] || return; for i in $(seq -w 1 99); do grep -q "#Yv$i" "$CONF" && { echo "Yv$i"; return; }; done
c=$(grep -oE -- '--comment=.*Yv[0-9]+' "$CONF" 2>/dev/null | head -n1); [ -n "$c" ] && echo "$c" | grep -oE 'Yv[0-9]+' | head -n1; }
_detect_discord_ver() { local d; [ -f "$CONF" ] || return; d=$(grep -o -E '^#[[:space:]]*Dv[0-9]+' "$CONF" 2>/dev/null | sed 's/^#[[:space:]]*//' | head -n1)
[ -n "$d" ] && { echo "$d"; return; }
if grep -qF -- '--filter-l7=discord' "$CONF" 2>/dev/null || grep -qF -- '--hostlist-domains=discord.media' "$CONF" 2>/dev/null; then echo "Dv1"; fi; }
show_current_strategy() { ver=$(_detect_base_ver); yv_ver=$(_detect_youtube_ver); }
...
echo -e "${CYAN}10) $S_ALL\n${CYAN}11) ${GREEN}Заменить ${NC}hosts${GREEN} на ${NC}GeoHide hosts"
echo -e "${CYAN}12) ${GREEN}Заменить ${NC}hosts${GREEN} на ${NC}Mafioznik hosts\n${CYAN}13) ${GREEN}Заменить ${NC}hosts${GREEN} на ${NC}Malw.link hosts\n${CYAN}14) ${GREEN}Восстановить ${NC}hosts"
...
1) $Z_ACTION_FUNC;; s|S) pgrep -f /opt/zapret >/dev/null 2>&1 && stop_zapret || start_zapret;; 2) menu_str;; 3)TEST_menu;; 4) menu_TG;; 5) DoH_menu;; 6) PODKOP_menu ;; 7) Discord_menu;; 8) menu_hosts;; 9) zapret_key;; 0) sys_menu;; *) echo; exit 0;; esac; };
